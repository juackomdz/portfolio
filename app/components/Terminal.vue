<template>
    <div ref="containerRef" :class="styles({ class: normalizeClass(props.class) || undefined })">
        <div class="border-border bg-background sticky top-0 left-0 z-10 flex flex-col gap-y-2 border-b p-4">
            <div class="flex flex-row gap-x-2">
                <div v-for="(item, i) in buttonColors" :key="i" class="size-2 rounded-full" :class="[item]" />
            </div>
        </div>
